...
def set_system_parametrs():
    ...
    params["VERBOSE_MODE"] = (
        "full"  # можно еще compact или конкретное значение символов
    )
...
def get_system_parameters(params):
    ...
    print(
        f"    Полоса частот: {params['f_start']/1000:.1f} - {params['f_end']/1000:.1f} кГц"
    )
    print(f"    Длительность символа: {params['T_sym']*1000:.1f} мс")
    print(
        f"    База сигнала B = {params['B']:.1f} (выигрыш {10*math.log10(params['B']):.1f} дБ)"
    )
    ...
    print(
        f"    Частотное затухание: {'ВКЛ' if params['status_attenuation'] else 'ВЫКЛ'}"
    )
    ...
    print(
        f"    Сигнал слабее шума в {10**(-params['SNR_target_dB']/10):.0f} раз по мощности"
    )
